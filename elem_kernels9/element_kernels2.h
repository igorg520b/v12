...
extern "C" __global__ void kelElementElasticityForce(
	...
	const int nElasticElems, const int el_elastic_stride, const int nd_stride,
	double *de)
{
	...
	F_and_Df_Corotational(x0, un, f, Df, V, sigma);
	de[idx + el_elastic_stride * 0] = sigma[0];
	de[idx + el_elastic_stride * 1] = sigma[1];
	de[idx + el_elastic_stride * 2] = sigma[2];
	de[idx + el_elastic_stride * 3] = sigma[3];
	de[idx + el_elastic_stride * 4] = sigma[4];
	de[idx + el_elastic_stride * 5] = sigma[5];
