...
namespace icFlow
{
    ...
    public class Mesh
    {
        ...
        const int eli_str = 5, eld_str = 9;                              // elements
        ...
        void LoadSaveMemAlloc()
        {
            ...
            dSize = nodes.Count * ndd_str + czs.Count * czd_str + elems.Count * eld_str;
        ...
        public void SaveFrame(BinaryWriter bw)
        {
            ...
            Parallel.For(0, elems.Count, i => {
                ...
                elem.id = i; 
                for(int j=0;j<4;j++) isn[i * eli_str + j] = elem.vrts[j].id;
                isn[i * eli_str + 4] = elem.granule;

                for(int j=0;j<6;j++) dsn[d_off + i * eld_str + j] = elem.stress[j];
                for(int j=0;j<3;j++) dsn[d_off + i * eld_str + 6+j] = elem.principal_stresses[j];
            });
            i_off += eli_str * elems.Count;
            d_off += eld_str * elems.Count;

            // cohesive zones
            Parallel.For(0, czs.Count, i => {
                CZ cz = czs[i];
                for(int j=0;j<6;j++) isn[i_off + i * czi_str + j] = cz.vrts[j].id;
        ...
        public void LoadFrame(Stream str, bool update = false)
        {
            ...
            Parallel.For(0, nElems, i => {
                ...
                for(int j=0;j<6;j++) elem.stress[j] = dsn[d_off + i * eld_str + j];
                for (int j = 0; j < 3; j++) elem.principal_stresses[j] = dsn[d_off + i * eld_str + 6 + j];
            ...
            foreach (SurfaceFragment sf in surfaceFragments) sf.RestoreNodes(nodes);
        ...
        void LoadMsh(Stream str)
        {
            ...
            while (s != "$EndElements")
            {
                ...
                {
                    case 2: // triangle
                        n0 = int.Parse(parts[sidx]) - 1;
                        n1 = int.Parse(parts[sidx+1]) - 1;
                        n2 = int.Parse(parts[sidx+2]) - 1;
                        Face f = new Face();
                        f.vrts[0] = nodes[n0];
                        f.vrts[1] = nodes[n1];
                        f.vrts[2] = nodes[n2];
            ...
            BoundingBox();
            ComputeVolume();
            DetectSurfacesAfterLoadingMSH(false);
        }

        public void DetectSurfacesAfterLoadingMSH(bool anchorsides)
        {
            ...
            // detect the largest grain (by volume) and associate it with a SurfaceFragment
            int maxGrains = elems.Max(elem => elem.granule) + 1;
            double[] grainVolumes = new double[maxGrains];
            foreach (Element e in elems) grainVolumes[e.granule] += e.volume;
            double maxVolume = grainVolumes.Max();
            int idx = -1;
            for(int i=0;i<=maxGrains;i++)
            {
                if(grainVolumes[i] == maxVolume)
                {
                    idx = i;
                    break;
                }
            }
            Debug.Assert(idx != -1,"identifying largest grain");
            SurfaceFragment largestGrain = new SurfaceFragment();
            largestGrain.name = "bulk";
            largestGrain.id = 6;
            surfaceFragments.Add(largestGrain);
            largestGrain._nodes = new HashSet<Node>();
            foreach (Element e in elems)
                if (e.granule == idx)
                    foreach (Node nd in e.vrts)
                        largestGrain._nodes.Add(nd);

        }

        #endregion

        #region compute volume, identify surface
        void ComputeVolume()
        {
            _volume = 0;
        ...
        public (double,double,double) CenterSample()
        {
        ...
        #region detect fracture

        public bool DetectFracture()
        {
            // list of element connections for nodes, CZs counted as their connecting elements
            Dictionary<Node, List<Element>> nec = new Dictionary<Node, List<Element>>();
            foreach (Node nd in nodes) nec.Add(nd, new List<Element>());

            foreach (CZ cz in czs)
                if (!cz.failed)
                    foreach (Node nd in cz.vrts)
                        foreach (Face fc in cz.faces)
                            nec[nd].Add(fc.elem);
            ...
            // now that "nec" is populated, build the fragments
            List<List<Element>> fragments = new List<List<Element>>();
            HashSet<Element> remainingElements = new HashSet<Element>(elems);

            Stack<Element> stack = new Stack<Element>(10000);
            do
            {
                List<Element> fragment = new List<Element>();
                fragments.Add(fragment);
                // pick first element and place it on the stack
                Element elem1 = remainingElements.First();
                stack.Push(elem1);
                remainingElements.Remove(elem1);
                do
                {
                    Element elem = stack.Pop();
                    fragment.Add(elem);
                    foreach(Node nd in elem.vrts)
                        foreach(Element elem2 in nec[nd])
                            if (remainingElements.Contains(elem2))
                            {
                                remainingElements.Remove(elem2);
                                stack.Push(elem2);
                            }
                } while (stack.Count > 0);
            ...
            Trace.WriteLine("Fragment volumes:");
            double[] fragmentVolumes = new double[fragments.Count];
            for(int i=0; i< fragments.Count; i++)
            {
                List<Element> fragment = fragments[i];
                double fragmentVolume = 0;
                foreach (Element elem in fragment) fragmentVolume += elem.volume;
                fragmentVolumes[i] = fragmentVolume;
                Trace.WriteLine($"({i}): {fragmentVolume}");
            }

            double totalV = fragmentVolumes.Sum();
            // see if at least two volumes are 10% of the total
            int tenPercentFragments = fragmentVolumes.Count(v => v > totalV * 0.1);
            if (tenPercentFragments > 1) return true;
            else return false;
        }
