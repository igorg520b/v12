...
namespace icFlow
{
    public class MeshCollection
    {
        ...
        public void Prepare()
        {
            ...
            /*
            // nodal volumes, for buoyancy
            foreach (Node nd in allNodes) nd.volume = 0;
            foreach(Element elem in elasticElements)
            {
                double volume = elem.volume;
                foreach (Node nd in elem.vrts) nd.volume += volume / 4;
            }
            */
        }
        ...
        public void Reset()
        {
            foreach (CZ cz in allCZs)
            {
                ...
                cz.failed = false;
                cz.avgDn = cz.avgDt = cz.avgTn = cz.avgTt = cz.maxAvgDn = cz.maxAvgDt = 0;
            }
        ...
        {
            mgs.Clear();
            deformables = nonDeformables = indenters = null;
            allNodes = activeNodes = null;
            surfaceElements = elasticElements = null;
            nonFailedCZs = allCZs = failedCZs = null;
            allFaces = null;
            exposedEdges = null;
        }
