...
//            public int id;
        ...
        public static void InsertCohesiveElements(this Mesh mg, bool anchorsides = false)
        {
            ...
            foreach (ExtendedFace f in innerTris)
            {
                ...
                if (ecz.sameGranule) countSameGrainCZs++;
                mg.czs.Add(ecz);
            }
            Console.WriteLine($"same grain czs {countSameGrainCZs}");
            ...
            mg.ConvertBack();
            mg.DetectSurfacesAfterLoadingMSH(anchorsides);
            for (int i = 0; i < mg.czs.Count; i++) mg.czs[i].immutableID = i;
        }
