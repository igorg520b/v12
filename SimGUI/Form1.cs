...
//            parametricStudyToolStripMenuItem_Click(sender, e);
        ...
        private void glControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (rprm.viewMode == RenderPrms.ViewMode.Model) PaintModel();
            else if (rprm.viewMode == RenderPrms.ViewMode.Stress) PaintStress();
        ...
        Font myFont2 = new Font("Tahoma", 11);
        ...
        void PaintStress()
        {
            ...
            // draw translucent shell
            int palette = rprm.Palette;

            (double, double, double) r = (0,0,0);
            if (model3.mc.deformables != null)
                foreach (Mesh msh in model3.mc.deformables)
                    r=msh.faces.RenderStress(rprm.WhichStress, palette);

            // draw grain edges (double-check coordinates used)
            if (model3.mc.exposedEdges != null)
            {
                model3.mc.exposedEdges.RenderGrainEdgesAsCylinders(
                    rprm.GrainBoundaryColor, rprm.CylinderSides, rprm.CylinderRadius);
                model3.mc.exposedEdges.RenderGrainEdges(rprm.GrainBoundaryWidth, rprm.GrainBoundaryColor);
            ...
            double span = max - min;
            if (span == 0) return;
            int nSteps = (int)Math.Round((decimal) (span / step));

            int w = glControl1.ClientSize.Width;
            int h = glControl1.ClientSize.Height;
            const float boxHeightPercent = 30f; // percents from the height of the control
            const float boxWidthPercent = 3f;

            float boxHeight = boxHeightPercent * 0.01f * h;
            float boxWidth = boxWidthPercent * 0.01f * w;

            const float boxMarginRightPercent = 6f;
            const float boxMarginBottomPercent = 6f;
            float boxRight = w * 0.01f * (100 - boxMarginRightPercent);
            float boxBottom = h * 0.01f * (100 - boxMarginBottomPercent);
            ...
            using (Bitmap bmp = new Bitmap(w, h))
            {
                ...
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    ...
                    {
                        double segmentFraction = (double)i / (segments - 1);
                        double value = max * (1 - segmentFraction) + min * segmentFraction;
                        double segmentFraction2 = (double)i / segments;
                        float segmentTop = (float)(boxTop * (1 - segmentFraction2) + boxBottom * segmentFraction2);
                        using (Brush brush = new System.Drawing.SolidBrush(MC.c3(value, min, max, palette)))
                        {
                            g.FillRectangle(brush, boxLeft, segmentTop, boxWidth, segmentHeight);
                        }
                    }
                    g.DrawRectangle(Pens.Black, boxLeft, boxTop, boxWidth, boxHeight);

                    // draw scale and text
                    float scaleWidth = 0.1f * boxWidth;
                    float scaleLeft = boxRight - scaleWidth / 2;
                    float scaleRight = boxRight + scaleWidth / 2;

                    for (int i=0;i<=nSteps;i++)
                    {
                        float segmentFraction = (float)i / (nSteps);
                        float scaleY = (float)(boxTop * (1 - segmentFraction) + boxBottom * segmentFraction);
                        g.DrawLine(Pens.Black, scaleLeft, scaleY, scaleRight, scaleY);

                        RectangleF rectf = new RectangleF(scaleRight+5, scaleY-10, 100, 50);
                        double value = max * (1 - segmentFraction) + min * segmentFraction;
                        g.DrawString(value.ToString("0.00e0"), myFont2, Brushes.Black, rectf, format2);
                    }
                ...
                data = bmp.LockBits(glControl1.ClientRectangle, ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
                GL.DrawPixels(w, h, OpenTK.Graphics.OpenGL.PixelFormat.Bgr, PixelType.UnsignedByte, data.Scan0);
                bmp.UnlockBits(data);
            }
        }
...
//            psf.TopMost = true;
        ...
        private void addToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            openFileDialogMeshes.Filter = "Mesh|*.geo;*.msh;*.mg;*.bin";
        ...
        private void writeCSVToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            FrameInfo.FrameSummary smr = new FrameInfo.FrameSummary(model3.allFrames, model3.prms);
        ...
        private void analysisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            fa2.smr = new FrameInfo.FrameSummary(model3.allFrames, model3.prms);
        ...
        bool timeToPause;
        private async void continueFromLastToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            do
            {
                await Task.Run(() => model3.Step());
                if (model3.prms.MaxSteps > 0 && model3.cf.StepNumber >= model3.prms.MaxSteps) timeToPause = true;
                if (model3.prms.DetectFracture && model3.cf.fractureDetected) timeToPause = true;
                // report progress
                glControl1.Invalidate();
                tnCurrentFrame.Tag = model3.cf;
                if (treeView1.SelectedNode == tnCurrentFrame)
        ...
        void TakeScreenshot(string filename, double fromTime, double toTime, double maxTime)
        {
            ...
            using (Bitmap bmp = new Bitmap(w, h))
            {
                ...
                // draw text and plots
                if(rprm.RenderText || rprm.RenderPlot)
                {
                    using(Graphics g = Graphics.FromImage(bmp))
                    {
                        g.CompositingMode = CompositingMode.SourceOver;
                        ...
                        if (model3.cf != null && rprm.RenderText) g.DrawString($"{rprm.Comment}\ntime: {toTime:0.000} s", myFont, Brushes.Black, rectf, format);
                        ...
                        fa.toTime = toTime;
                        fa.maxTime = maxTime;
                        fa.smr = new FrameInfo.FrameSummary(model3.allFrames, model3.prms);
                        fa.ShowData();

                        Rectangle bounds = new Rectangle(w-w2, h-h2, w, h);
                        if(rprm.RenderPlot) fa.chart1.DrawToBitmap(bmp, bounds);
        ...
        private void setupGenerationToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // (1) display L-beam meshing tool
            // (2) if OK, then import beam mesh and set up indenter mesh
            using (IcyGrains.SetupGeneratorForm sgf = new IcyGrains.SetupGeneratorForm())
            {
                if (sgf.ShowDialog() == DialogResult.OK)
                    SetUpBeamSimulation(sgf.memStr, sgf.memStrIndenter, sgf.rprm);
            }
            glControl1.MakeCurrent();
        }

        // setup the simulation based on beam and indenter generated by IcyGrains tool
        public void SetUpBeamSimulation(Stream strBeam, Stream strIndenter, 
            IcyGrains.BeamParams beamParams, ModelPrms modelParams = null)
        {
            model3.Clear();
            if (modelParams != null) model3.prms = modelParams;
            else
            {
                if (beamParams.type == IcyGrains.BeamParams.BeamType.Plain)
                {
            ...
            Mesh mgIndenter = new Mesh(strIndenter, "indenter", ".msh");
            mg.isDeformable = true;
